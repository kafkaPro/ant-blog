// =================================================================================
// This is auto-generated by GoFrame CLI tool only once. Fill this file as you wish.
// =================================================================================

package dao

import (
	"blog-backend/app/dao/internal"
)

// operateLogDao is the manager for logic model data accessing and custom defined data operations functions management.
// You can define custom methods on it to extend its functionality as you wish.
type operateLogDao struct {
	*internal.OperateLogDao
}

var (
	// OperateLog is globally public accessible object for table operate_log operations.
	OperateLog = operateLogDao{
		internal.NewOperateLogDao(),
	}
)

// Fill with you ideas below.
